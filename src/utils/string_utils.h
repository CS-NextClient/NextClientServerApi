...
namespace str_utils
{
    ...
    {
        std::for_each(s.begin(), s.end(), [](char& c) { c = _fast_to_lower(c); });
    }
    ...
    [[nodiscard]] inline std::string to_lower_copy(const std::string& s)
    {
        ...
        std::for_each(result.begin(), result.end(), [](char& c) { c = _fast_to_lower(c); });
    ...
    [[nodiscard]] inline bool contains(
        ...
        CompareOptions compare_options = CompareOptions::None
    )
    {
        if (compare_options == CompareOptions::RegisterIndependent)
        {
            auto found = std::search(str.begin(), str.end(), sub.begin(), sub.end(), [](char a, char b) {
                return _fast_to_lower(a) == _fast_to_lower(b);
            });
    ...
    [[nodiscard]] inline bool equals(const std::string& a, const std::string& b, CompareOptions compare_options = CompareOptions::None)
    {
        if (compare_options == CompareOptions::RegisterIndependent)
        {
            return a.size() == b.size() &&
                   std::equal(a.begin(), a.end(), b.begin(), [](char a, char b) { return _fast_to_lower(a) == _fast_to_lower(b); });
        }
    ...
    [[nodiscard]] inline bool start_with(
        const std::string& str,
        const std::string& sub,
        CompareOptions compare_options = CompareOptions::None
    )
    {
        if (compare_options == CompareOptions::RegisterIndependent)
        {
            if (sub.size() > str.size())
                return false;
            auto it = std::mismatch(str.begin(), str.begin() + sub.size(), sub.begin(), [](char a, char b) {
                return _fast_to_lower(a) == _fast_to_lower(b);
            });
